...
function getMyLocation () { //собственно наша функция для определения местоположения
	if (navigator.geolocation) { //для начала надо проверить, доступна ли геолокация, а то еще у некоторых браузеры то древние. Там о таком и не слышали.
		...
		alert("Oops, geolocation is not supported"); //выведем сообщение для старых браузеров.
	}
}
if (!flag)
{
            ...
            if (flag)
            {
                ...
                .then(function(data)
                {
                    ...
                    for (let i = 0; i < 25; i++)
                    {
                        ...
                        document.getElementById(`elem-vis${i}`).textContent = "Visibility " + Math.round((data.hourly[i].visibility)/1000) + ' km';
                    }
                    ...
                    for (i = 1; i < 8; i++)
                    {
                        ...
                        document.getElementById(`sd-wind-gusts${i}`).textContent = 'до ' + Math.round(data.daily[i].wind_gust) +' m/s';
                        ...
                        document.getElementById(`seven-days-vis${i}`).textContent = Math.round((data.hourly[i].visibility)/1000) + ' km';
                    ...
                    let url = `https://api.weatherbit.io/v2.0/forecast/hourly?lat=${latitude}&lon=${longitude}&key=${apiKey2}&hours=193&lang=en`
                    fetch(url)
                        .then(function(resp){return resp.json() })
                        .then(function(dataW)
                        { 
